...
const COMMITMENT_ICONS: Record<(typeof COMMITMENT_KEYS)[number], ReactNode> = {
  ...
  artisanal: (
    <path d="M4 18h16M7 18V9l5-4 5 4v9M10 18v-4h4v4" />
  ),
};
    ...
    <section className="bg-primary text-background">
      <div className="mx-auto max-w-7xl space-y-10 px-4 py-14 md:px-6 lg:py-16">
        <div className="mx-auto max-w-2xl space-y-3 text-center">
          ...
          <p className="text-background/80">{t("about.commitments.subtitle")}</p>
        </div>

        <div className="grid gap-8 sm:grid-cols-3">
          {COMMITMENT_KEYS.map((key) => (
            <div key={key} className="space-y-4 text-center sm:text-left">
              <div className="mx-auto inline-flex size-11 items-center justify-center rounded-2xl bg-background/10 text-accent-light sm:mx-0">
                <svg
                  ...
                  strokeWidth="1.75"
                >
                  {COMMITMENT_ICONS[key]}
                </svg>
              </div>
              <h3 className="font-serif text-xl tracking-wide">
                {t(`about.commitments.items.${key}.title`)}
              </h3>
              <p className="text-sm leading-relaxed text-background/80">
                {t(`about.commitments.items.${key}.description`)}
              </p>
            </div>
          ))}
        </div>
