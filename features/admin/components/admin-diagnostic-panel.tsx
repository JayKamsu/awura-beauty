...
export function AdminDiagnosticPanel() {
  ...
  const toggleQuestion = async (question: DiagnosticQuestion) => {
    await adminFetch("/api/admin/diagnostic/questions", {
      ...
      body: JSON.stringify({
        question: {
          ...
          subtitleFr: question.subtitleFr,
          subtitleEn: question.subtitleEn,
          subtitleEs: question.subtitleEs,
        },
      }),
    });
    await load();
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="font-medium text-primary">
                    {q.titleFr || q.questionKey}
                  </p>
                  <p className="text-xs text-muted">
                    {q.channel} · {q.questionKey} · {q.options.length}{" "}
                ...
                >
                  {q.enabled
                    ? t("admin.diagnostic.disable")
                    : t("admin.diagnostic.enable")}
                </Button>
              </div>
        ...
        <div className="grid max-w-xl gap-4">
          {(
            ...
            >
              ...
              <label className="block space-y-1 text-sm">
                <span className="text-muted">
                  {t("admin.diagnostic.compareCents")}
                </span>
                <input
                  type="number"
                  className="w-full rounded-xl border border-border bg-background px-3 py-2"
                  value={settings[compareKey]}
                  onChange={(e) =>
                    setSettings({
                      ...settings,
                      [compareKey]: Number(e.target.value),
                    })
                  }
                />
              </label>
          ...
          <label className="block space-y-1 text-sm">
            <span className="text-muted">
              {t("admin.diagnostic.slotDuration")}
            </span>
            <input
              type="number"
              className="w-full rounded-xl border border-border bg-background px-3 py-2"
          ...
          <label className="block space-y-1 text-sm">
            ...
            <textarea
              ...
              onChange={(e) =>
                setSettings({
                  ...settings,
                  physicalLocationText: e.target.value,
                })
              }
            />
          </label>
              ...
              return (
                ...
                >
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const next = rules.filter((r) => r.weekday !== weekday);
                      next.push({ ...rule, startTime: e.target.value, enabled: true });
                      setRules(next);
                    }}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const next = rules.filter((r) => r.weekday !== weekday);
                      next.push({ ...rule, endTime: e.target.value, enabled: true });
                      setRules(next);
                    }}
                    className="rounded-lg border border-border px-2 py-1"
                  />
          ...
          <div className="space-y-3 border-t border-border pt-5">
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              ...
              >
                ...
                <option value="open">
                  {t("admin.diagnostic.overrideOpen")}
                </option>
              </select>
        ...
        <div className="space-y-6">
          <div className="space-y-3 rounded-2xl border border-border p-4">
            ...
            <p className="text-sm text-muted">
              {t("admin.diagnostic.sendResultHint")}
            </p>
            <label className="block space-y-1 text-sm">
              <span className="text-muted">
                {t("admin.diagnostic.linkAppointment")}
              </span>
              <select
                className="w-full rounded-xl border border-border bg-background px-3 py-2"
              ...
              >
                <option value="">
                  {t("admin.diagnostic.noAppointment")}
                </option>
                {appointments.map((a) => (
            ...
            <label className="block space-y-1 text-sm">
              <span className="text-muted">
                {t("admin.diagnostic.resultSummary")}
              </span>
              <textarea
                className="min-h-24 w-full rounded-xl border border-border bg-background px-3 py-2"
            ...
            <label className="block space-y-1 text-sm">
              <span className="text-muted">
                {t("admin.diagnostic.detailedFeedback")}
              </span>
              <textarea
          ...
          <div className="overflow-x-auto rounded-2xl border border-border">
            ...
            <table className="min-w-full text-left text-sm">
              <thead className="bg-background-alt text-xs uppercase text-muted">
                <tr>
                  <th className="px-3 py-2">{t("admin.colDate")}</th>
                  <th className="px-3 py-2">{t("admin.colClient")}</th>
                  <th className="px-3 py-2">{t("admin.colPayment")}</th>
                    ...
                    <td className="px-3 py-2 text-right">
                      ...
                      {a.status === "confirmed" ? (
                        <Button
                          ...
                          variant="ghost"
                          onClick={() =>
                            void updateAppointmentStatus(a.id, "completed")
                          }
                        >
                      ...
                      a.status === "confirmed" ? (
                        <Button
                          ...
                          variant="ghost"
                          onClick={() =>
                            void updateAppointmentStatus(a.id, "cancelled")
                          }
                        >
